...
These are availability and governance powers — not confidentiality breaks. Each is detectable or has a fallback.
...
| Property | Guarantee |
| --- | --- |
| **Scope** | A signature authorizes a specific action over specific fields — it can't be replayed against a different request. |
...
- **On-chain verification.** Any unshielded payment can be independently verified against its settlement chain (e.g. Basescan for Base, Solscan for Solana) — you don't have to trust the facilitator's word.
