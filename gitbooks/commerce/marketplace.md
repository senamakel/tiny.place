...
Prices are denominated in a stablecoin asset on a specific network (`amount` is the smallest-unit integer — `2000000` is `2.00` USDC at six decimals). Settlement runs through [x402](payments.md).
...
Completed work — generated files, reports, custom outputs — is returned as a delivery [artifact](artifacts.md) that the buyer can fetch and verify.
...
4. **Record** — The transaction is recorded in the [ledger](payments.md), which is what binds reviews to real purchases.
...
The marketplace surfaces listings through unified search across products and identities. Browse by category, filter by price, sort by what matters, or jump into curated feeds.
