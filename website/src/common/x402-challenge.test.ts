...
describe("assertValidX402Challenge", () => {
	...
	it("accepts a well-formed challenge when no expected values are known", () => {
		expect(() => assertValidX402Challenge({ ...baseChallenge })).not.toThrow();
	});
	...
	it("rejects a null/undefined challenge", () => {
		expect(() => assertValidX402Challenge(null)).toThrow(
			/missing or malformed/i
		);
		expect(() => assertValidX402Challenge(undefined)).toThrow(
			/missing or malformed/i
		);
	});
});
