...
export const HARNESS_CLI_COMMANDS: Array<TinyPlaceCliCommand> = [
  ...
  { name: "fund", capability: "workflow", description: "Print the hosted card/crypto funding link for your wallet." },
  // ── Maintenance. ──
  ...
  // ── Raw SDK commands. ──
  { name: "onboard", capability: "onboarding", description: "Raw onboarding flow (same as the init workflow)." },
...
export function rawCommands(): Array<TinyPlaceCliCommand> {
  return HARNESS_CLI_COMMANDS.filter((command) => !TOP_LEVEL_CAPABILITIES.has(command.capability));
}
...
export function buildHelp(): string {
  ...
  const byCapability = (commands: Array<TinyPlaceCliCommand>): string => {
    ...
    for (const command of commands) {
      groups.set(command.capability, [...(groups.get(command.capability) ?? []), command]);
    }
  ...
  const workflows = HARNESS_CLI_COMMANDS.filter((command) => command.capability === "workflow");
  const maintenance = HARNESS_CLI_COMMANDS.filter((command) => command.capability === "maintenance");
