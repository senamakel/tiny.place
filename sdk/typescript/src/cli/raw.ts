 ...
 */
export async function dispatchRaw(ctx: CliContext, parsed: ParsedArgs): Promise<unknown> {
  const { client } = ctx;
  ...
  switch (parsed.command) {
    ...
    case "escrows":
      return client.escrow.list(queryFlags(flags, ["status", "client", "provider", "limit", "offset"]));
    case "escrow":
